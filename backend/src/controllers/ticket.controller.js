...
export const getTicketByBooking = async (req, res) => {
  try {
    ...
    logger.error('Get ticket by booking error:', error);
    res.status(404).json({
      success: false,
      message: error.message || 'Không tìm thấy vé',
    });
 ...
 */
export const requestTicketLookupOTP = async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
 ...
 */
export const verifyTicketLookupOTP = async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
...
export const getTripPassengers = async (req, res) => {
  try {
    ...
    logger.error('Get trip passengers error:', error);
    res.status(500).json({
      success: false,
...
export const changeTicket = async (req, res) => {
  try {
    ...
    logger.error('Change ticket error:', error);
    res.status(400).json({
      success: false,
...
export const resendTicket = async (req, res) => {
  try {
    ...
    logger.error('Resend ticket error:', error);
    res.status(500).json({
      success: false,
