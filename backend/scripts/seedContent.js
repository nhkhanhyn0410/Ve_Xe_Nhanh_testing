...
const BLOGS = [
  ...
  },
  {
    ...
    content:
      ...
      tip('GỢI Ý', 'Báo phụ xe trước khi tới trạm dừng để giữ chỗ — chuyến đông khách thời gian dừng chỉ 15 — 20 phút.'),
  },
  {
    ...
    content:
      ...
      tip('LƯU Ý', 'Khai báo hành lý có giá trị cao khi gửi để được hỗ trợ bồi thường theo chính sách của hãng.'),
  },
  {
    ...
    content:
      ...
      tip('MẸO NHỎ', 'Đi xe đêm từ Hà Nội, tới Sapa 5h sáng là vừa kịp ăn sáng rồi bắt đầu cung trekking đầu tiên.'),
  },
  {
    slug: 'khuyen-mai-he-vxn-2026',
    title: 'Khuyến mãi hè VXN: giảm đến 35% toàn bộ tuyến miền Bắc',
    excerpt:
      'Từ 15/05 đến 31/07/2026, áp dụng cho 50.000 vé đầu tiên. Đặt sớm để chọn ghế đẹp.',
    category: 'promotion',
    ...
    content:
      ...
      tip('ĐẶT SỚM', 'Số lượng có hạn — đặt sớm vừa được giá tốt vừa chọn được ghế tầng dưới, vị trí giữa xe.'),
  },
  {
    ...
    content:
      ...
      tip('CHỌN NHANH', 'Chuyến dưới 5 giờ ban ngày → limousine. Chuyến đêm trên 6 giờ → giường nằm cabin VIP.'),
  },
  {
    ...
    content:
      ...
      tip('QUAN TRỌNG', 'Vé giữ tạm chỉ trong thời gian giới hạn — chuẩn bị sẵn phương thức thanh toán để không mất chỗ.'),
  },
  {
    ...
    content:
      ...
      tip('TRẢI NGHIỆM', 'Đặt thử tuyến mới trong tháng ra mắt để nhận ưu đãi giới thiệu dành cho khách tiên phong.'),
  },
];

const FAQS = [
  // Đặt vé
  ['booking', 'Đặt vé qua VXN có cần đăng ký tài khoản không?',
    'Bạn có thể đặt vé với tư cách khách (guest) chỉ với số điện thoại và email. Tuy nhiên, đăng ký tài khoản giúp bạn tích điểm thưởng, tra cứu vé nhanh và lưu danh sách hành khách thường đi.', 1, ['tài khoản', 'guest']],
  ['booking', 'Có thể đặt vé cho người khác đi không?',
    'Hoàn toàn được. Ở bước nhập thông tin hành khách, bạn chỉ cần điền đúng họ tên và số điện thoại của người đi thực tế. Vé điện tử sẽ được gửi tới email/điện thoại bạn đăng ký khi đặt.', 2, ['đặt hộ']],
  ['booking', 'Đặt tối đa bao nhiêu vé trong một lần?',
    'Mỗi lượt đặt hỗ trợ tối đa số ghế còn trống của chuyến, thông thường tới 10 vé/giao dịch. Với đoàn lớn hơn, vui lòng liên hệ CSKH để được hỗ trợ đặt theo đoàn.', 3, ['nhóm', 'đoàn']],
  ['booking', 'Vé giữ tạm thời trong bao lâu trước khi thanh toán?',
    'Sau khi chọn ghế, hệ thống giữ chỗ cho bạn trong một khoảng thời gian giới hạn để hoàn tất thanh toán. Hết thời gian, ghế sẽ được mở lại cho khách khác.', 4, ['giữ ghế']],
  ['booking', 'Trẻ em có cần mua vé riêng không?',
    'Quy định theo từng nhà xe. Thông thường trẻ dưới một chiều cao/độ tuổi nhất định được miễn vé khi ngồi cùng người lớn; trẻ lớn hơn cần mua vé như người lớn. Chi tiết hiển thị ở trang chính sách của chuyến.', 5, ['trẻ em']],

  // Thanh toán
  ['payment', 'VXN hỗ trợ những phương thức thanh toán nào?',
    'VXN hỗ trợ thanh toán qua cổng VNPay (thẻ ngân hàng nội địa, thẻ quốc tế, ví điện tử và QR). Một số tuyến hỗ trợ thanh toán khi lên xe tùy chính sách nhà xe.', 1, ['vnpay', 'thẻ']],
  ['payment', 'Thanh toán bị trừ tiền nhưng chưa nhận được vé thì sao?',
    'Trong đa số trường hợp vé sẽ được xác nhận trong vài phút. Nếu sau 15 phút chưa nhận được, vui lòng kiểm tra mục Hành trình hoặc liên hệ CSKH kèm mã giao dịch — khoản tiền sẽ được đối soát và hoàn nếu giao dịch không thành công.', 2, ['lỗi thanh toán']],
  ['payment', 'Tôi có nhận được hoá đơn VAT không?',
    'Có. Bạn có thể yêu cầu xuất hoá đơn điện tử cho đơn hàng trong mục chi tiết vé hoặc liên hệ CSKH trong vòng thời gian quy định kể từ ngày đi.', 3, ['hoá đơn', 'vat']],

  // Đổi & huỷ vé
  ['cancellation', 'Tôi có thể huỷ vé và được hoàn tiền không?',
    'Chính sách hoàn huỷ phụ thuộc từng nhà xe và thời điểm huỷ so với giờ khởi hành. Mức hoàn cụ thể được hiển thị rõ ở bước thanh toán và trong chi tiết vé.', 1, ['hoàn tiền', 'huỷ']],
  ['cancellation', 'Đổi giờ/đổi ngày chuyến đi như thế nào?',
    'Vào mục Hành trình, chọn vé cần đổi và làm theo hướng dẫn nếu nhà xe cho phép đổi. Phí đổi (nếu có) và khung thời gian được áp dụng theo chính sách hiển thị trên vé.', 2, ['đổi vé']],
  ['cancellation', 'Bao lâu thì tiền hoàn về tài khoản của tôi?',
    'Sau khi yêu cầu hoàn được duyệt, thời gian tiền về phụ thuộc ngân hàng/cổng thanh toán, thường trong vài ngày làm việc.', 3, ['thời gian hoàn']],

  // Tài khoản
  ['account', 'Làm sao để đặt lại mật khẩu khi quên?',
    'Tại màn hình đăng nhập, chọn "Quên mật khẩu", nhập email/số điện thoại đã đăng ký và làm theo hướng dẫn nhận mã xác thực để tạo mật khẩu mới.', 1, ['mật khẩu']],
  ['account', 'Tôi có thể đổi số điện thoại/email tài khoản không?',
    'Được. Vào Hồ sơ cá nhân để cập nhật thông tin. Một số thay đổi quan trọng có thể cần bước xác thực để bảo vệ tài khoản.', 2, ['hồ sơ']],

  // Vé điện tử
  ['tickets', 'Vé điện tử của tôi nằm ở đâu?',
    'Vé điện tử được gửi qua email/SMS sau khi thanh toán thành công và luôn có trong mục Hành trình của tài khoản. Bạn chỉ cần xuất trình mã QR khi lên xe.', 1, ['vé điện tử', 'qr']],
  ['tickets', 'Tôi có cần in vé giấy không?',
    'Không cần. Chỉ cần xuất trình mã QR trên điện thoại để nhân viên/quản lý chuyến quét khi lên xe.', 2, ['in vé']],
  ['tickets', 'Tra cứu vé khi đặt với tư cách khách (guest) thế nào?',
    'Dùng chức năng Tra cứu vé, nhập mã vé/đặt chỗ cùng số điện thoại đã dùng khi đặt để xem chi tiết và mã QR.', 3, ['tra cứu', 'guest']],

  // Tuyến đường
  ['routes', 'Làm sao biết điểm đón/trả cụ thể của chuyến?',
    'Điểm đón và điểm trả được hiển thị chi tiết ở trang chọn chuyến và trong chi tiết vé. Một số nhà xe có hỗ trợ trung chuyển — thông tin sẽ ghi rõ kèm theo.', 1, ['điểm đón', 'trung chuyển']],
  ['routes', 'Xe có dừng nghỉ dọc đường không?',
    'Các chuyến đường dài thường có 1 — 2 điểm dừng nghỉ. Thời gian và vị trí dừng tùy theo lịch trình của nhà xe, phụ xe sẽ thông báo trước khi tới trạm.', 2, ['điểm dừng']],

  // Chính sách
  ['policies', 'Quy định về hành lý ký gửi và xách tay ra sao?',
    'Thông thường mỗi khách được mang 20kg hành lý ký gửi và 1 kiện xách tay không quá 7kg. Phần vượt và vật phẩm hạn chế áp dụng theo chính sách của từng nhà xe.', 1, ['hành lý']],
  ['policies', 'Mang theo thú cưng có được không?',
    'Tùy chính sách từng nhà xe. Nếu được phép, thú cưng cần có lồng/giỏ phù hợp và tuân thủ quy định vệ sinh, an toàn cho hành khách khác.', 2, ['thú cưng']],

  // Kỹ thuật
  ['technical', 'Ứng dụng/website báo lỗi khi đặt vé, tôi nên làm gì?',
    'Hãy thử tải lại trang, kiểm tra kết nối mạng hoặc cập nhật ứng dụng lên bản mới nhất. Nếu vẫn lỗi, liên hệ CSKH kèm ảnh chụp màn hình để được hỗ trợ nhanh.', 1, ['lỗi', 'bug']],
  ['technical', 'Tôi không nhận được email/SMS xác nhận vé?',
    'Kiểm tra hộp thư rác/quảng cáo và đảm bảo email/số điện thoại nhập đúng. Vé luôn có sẵn trong mục Hành trình; nếu cần gửi lại, liên hệ CSKH.', 2, ['không nhận email']],
];

(async () => {
  await mongoose.connect(
    process.env.MONGODB_URI || 'mongodb://localhost:27017/quikride',
